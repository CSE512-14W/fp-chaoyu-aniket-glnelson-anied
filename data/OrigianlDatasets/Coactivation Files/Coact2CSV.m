...
% Load the Data
file = load('Coactivation_matrix.mat');
edge_matrix = file.Coactivation_matrix;
...
frame = getframe(h);
filename = 'Coactivation_vertices_clust10_dendrogram.png';
...
% colorbar;
title('Coactivation Edge Strength');
xlabel('Destination Node #');
ylabel('Source Node #');
% set(h, 'XAxisLocation', 'top');
frame = getframe(h);
filename = 'Coactivation_edge_values.png';
imwrite(frame.cdata, filename, 'png');
...
% colorbar;
% caxis([min(edge_matrix(:)) max(edge_matrix(:))]);
title('Coactivation Edge Strength (Arranged by Clustering)');
% xlabel('Node #');
% ylabel('Node #');
% set(ha, 'XAxisLocation', 'top');
axis('off')
frame = getframe(h);
filename = 'Coactivation_edge_values_arrangedbyclustering.png';
imwrite(frame.cdata, filename, 'png');
...
title('Coactivation Points');
xlabel('X: Right(+) to Left(-)');
ylabel('Y: Anterior(+) to Posterior(-)');
zlabel('Z: Superior(+) to Inferior(-)');
view(3)
frame = getframe(h);
filename = 'Coactivation_vertices.png';
imwrite(frame.cdata, filename, 'png');
view(2)
frame = getframe(h);
filename = 'Coactivation_vertices_overhead.png';
imwrite(frame.cdata, filename, 'png');
...
view(3)
frame = getframe(h);
filename = 'Coactivation_vertices_clust10.png';
imwrite(frame.cdata, filename, 'png');
view(2)
frame = getframe(h);
filename = 'Coactivation_vertices_clust10_overhead.png';
imwrite(frame.cdata, filename, 'png');
...
title('Coactivation Edges');
xlabel('X: Right(+) to Left(-)');
ylabel('Y: Anterior(+) to Posterior(-)');
zlabel('Z: Superior(+) to Inferior(-)');
view(3)
frame = getframe(h);
filename = 'Coactivation_edges.png';
imwrite(frame.cdata, filename, 'png');
view(2)
frame = getframe(h);
filename = 'Coactivation_edges_overhead.png';
imwrite(frame.cdata, filename, 'png');

%% Save Data as CSV

struct2csv(V, 'Coactivation_V.csv');
struct2csv(E, 'Coactivation_E.csv');
