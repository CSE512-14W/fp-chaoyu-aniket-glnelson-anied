...
for c = 1:length(subconditions)
    ...
    for ir = 1:N
        for jr = 1:M
            data(ir, jr, :) = act(ir, jr, randperm(T));
        end
    end
...
results_skew = skewness(flat_results);
results_kurt = kurtosis(flat_results);
titlestr = sprintf('PTC3 %s GCI Null Distribution (Cortical Activity Randomized within nodes)\nmu: %.2f    std: %.2f    skew: %.2f    kurt:%.2f', condition, results_mean, results_std, results_skew, results_kurt);
title(titlestr);
ylabel('Count of GCI');
xlabel('GCI Bins');
ylim([0, 3e5]);
xlim([-1.5, 1.5]);
frame = getframe(gcf);
filename = sprintf('PTC3_%s_gci_nullwinwaves_distribution.png', condition);
imwrite(frame.cdata, filename, 'png');

save('PTC3_words_nullwinwaves.mat');
